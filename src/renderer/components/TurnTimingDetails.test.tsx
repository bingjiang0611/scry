...
describe('TurnTimingDetails', () => {
  it('只展示模型响应耗时与工具调用耗时，并按整轮墙钟互斥划分', () => {
    ...
    expect(html).toContain('工具调用耗时</span><b>5.8s</b><em>整轮剩余')
  ...
  it('模型响应存在并发时使用区间并集，不再展示累计与重叠', () => {
    ...
    expect(html).toContain('工具调用耗时</span><b>2.0s</b><em>整轮剩余')
  ...
  it('调用缺少生命周期时不制造耗时', () => {
    ...
    expect(html).toContain('工具调用耗时</span><b>—</b><em>未计时')
    expect(html).not.toContain('0ms')
  })

  it('缺少响应边界时保留模型估算，并把墙钟余量归入工具调用', () => {
    const bash = event({
    ...
    expect(html).toContain('工具调用耗时</span><b>3.0s</b><em>整轮剩余')
  ...
  it('没有工具调用时明确显示零耗时', () => {
    ...
    expect(html).toContain('工具调用耗时</span><b>0ms</b><em>本轮无工具调用')
  })
})
