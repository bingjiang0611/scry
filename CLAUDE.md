...
11. **打包 App 固定私有 Scry CLI，开发环境才回退 PATH**：安装包必须包含 `Resources/bin/scry` 与同源 recorder dist，wrapper 通过 `ELECTRON_RUN_AS_NODE=1` 复用 App 自带 Electron runtime。`runtimeCliEnv()` 优先把该绝对路径写入 `SCRY_CLI_PATH`；只有开发环境缺少内置产物时才从原始登录 shell PATH / 用户级 fallback 查找外部 CLI，并继续执行精确版本校验。显式 `SCRY_CLI_PATH` 仍是调试覆盖且保持 authoritative；路径失效时 hook 应 fail-open，不能偷偷切换到 Provider 改写后的 PATH。
12. **右侧终端是真 PTY，不是 `child_process` pipe**：main 通过 `node-pty` 管理本机 Shell；绑定项目时从当前 cwd 启动，未绑定时由 main 固定从用户主目录启动，renderer 不能任选初始目录。renderer 只持有 preload 暴露的窄 IPC 和 xterm 视图。`node-pty` 是 Electron ABI 原生模块，`electron-builder.yml` 必须持续 `asarUnpack`；`postinstall` 的 `scripts/prepare-node-pty.mjs` 保证 macOS `spawn-helper` 可执行。终端输出只驻留内存，不写 trace/SQLite；关闭 PTY 遵循正常终端 SIGHUP 语义，不要声称能杀掉主动忽略 SIGHUP 或已脱离终端控制的派生进程。
