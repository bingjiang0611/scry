...
10. **不跑任务拿 MCP 状态 / 斜杠命令清单（`captureInit`，agent-runner.ts）**：起最小 `query('hi')` 读 init。① MCP `mcp_servers` 和斜杠命令在 **init ~2.5s** 就到，但 `'hi'` 跑完要 **~12.5s**——所以 slash 菜单用 `waitFor:'init'` 提前 resolve（剩下流在后台读完），MCP ↻ 用 `'result'` 等 stdio pending 收敛。② **不能 `break`/`interrupt` 提前退**（触发 SDK `ProcessTransport not ready` 悬挂报错），只能让流自然读完。③ 命令清单来自 `q.supportedCommands()`，**只能在流里（init 后 transport 活着）调**，流结束后调会 `Query closed`；返回 `{name,description,argumentHint}`（含内置 + skill）。④ MCP / 命令都是 **cwd 相关**，`setCwd` 切目录时 `lastMcpStatus`/`lastSlashCommands` 缓存要失效。⑤ OAuth server 的工具清单 app 拿不到——裸握手测无 token 返 401，init 只给 `{name,status}` 不带工具名，所以「查看工具」按钮对 OAuth server 不出现（终端已认证才列得出）。
